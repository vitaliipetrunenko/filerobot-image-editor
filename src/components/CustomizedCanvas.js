...
export default class CustomizedCanvas extends Component {
  ...
  _allowedTabs = ["shapes", "image", "text", "watermark"];
  ...
  componentDidMount() {
    if (this.canvasRef && this.canvasRef.current && !this._canvas) {
      ...
      const border = `1px solid ${
        ...
        (this.props.colorScheme === "light" ? "#000" : "#fff")
      }`;
      const availableShapes = [
        ...
        },
        {
          ...
          iconStyles: { border },
          drawFn: (props) =>
            this.addSquare({ width: 75, height: 75, ...props }),
          // iconUrl: undefined,
        },
        {
          label: "Circle",
  ...
  static getDerivedStateFromProps(props, state) {
    return {
      ...state,
      latestCanvasSize: props.latestCanvasSize || { width: 0, height: 0 },
    };
  }
  ...
  prepareFinalCanvas = (originalCanvasDimensions) => {
    ...
    shapes.map((shape) => {
      ...
      if (shape.variant !== SHAPES_VARIANTS.TEXT) {
        ...
        shape.textSize = parseInt(shape.textSize).mapNumber(
          0,
          oldCanvas.width,
          0,
          width
  ...
  targettedShape = (index = undefined) =>
    index || index === 0 ? this.props.shapes[index] : this.props.selectedShape;
  ...
  activateResizingActions = () => {
    ...
    Array.from(
      document.getElementsByClassName("shape-resizing-control")
    ).forEach((control) => {
  ...
  handleShapeResizing = ({ movementX, movementY, shiftKey }) => {
    ...
    let { index, width, height, x, y, variant, originalWidth, originalHeight } =
      selectedShape;
    ...
    const keepShapeRatio = (sameAxesIncSign) => {
      ...
      if (Math.abs(movementX) >= Math.abs(movementY)) {
        ...
        movementY =
          (!sameAxesIncSign ? -1 : 1) * Math.sign(movementX) * tempHeight;
      } else {
        const tempWidth = Math.abs(width - (height + movementY) * ratio);
        movementX =
          (!sameAxesIncSign ? -1 : 1) * Math.sign(movementY) * tempWidth;
      }
    };
    const eastHandle = () => {
      width += movementX;
    ...
    switch (direction) {
      ...
      case "ne":
        ...
        eastHandle();
        if (width >= this._canvas.width && shiftKey) {
          break;
        }
        northHandle();
        break;
    ...
    if (x + width > this._canvas.width) {
      x = this._canvas.width - width;
    }
    if (y + height > this._canvas.height) {
  ...
  startDragging = (event) => {
    ...
    if (newX + width > this._canvas.width) {
      newX = this._canvas.width - width;
    }
    if (newY + height > this._canvas.height) {
      newY = this._canvas.height - height;
    }
    ...
    this.updateShape(newSelectedShape, index, {
  ...
  addRect = ({
    ...
    if (others.key && this.replaceShapeIfExisted(others.key, allArgs)) {
      return;
    }
  ...
  addImage = ({
    ...
    if (img) {
      const addIt = () => {
        const [originalWidth, originalHeight] = this.getSuitableImgDiemensions(
          img,
          others.lockScaleToPercentage
        );
        ...
        if (
          others.key &&
          this.replaceShapeIfExisted(others.key, allArgs, otherStates)
        ) {
          return;
        }
  ...
  addText = ({
    ...
    const [width, height] = this.getTextWidthAndHeight({
      text,
      textSize,
      textFont,
    });
  ...
  getShapeByKeyOrIndex = ({ key: shapeKey, index: shapeIndex }) => {
    ...
    return shapeIndex
      ? shapes[shapeIndex]
      : shapes.filter(({ key }) => key === shapeKey)[0];
  };
  ...
  getShapesIndexByAnyProp = (propertyName, propertyValue) => {
    ...
    shapes.filter(({ [propertyName]: filterProp }, index) => {
      if (
        filterProp === propertyValue ||
        (typeof filterProp === "undefined" &&
          Boolean(filterProp) === propertyValue)
      ) {
  ...
  updateShape = (updatedData, index, otherStatesToBeUpdated = undefined) => {
    ...
    if (!index && index !== 0) {
      updates.selectedShape = { ...selectedShape, ...updatedData };
    }
    ...
    if (shapes[index]) {
      ...
      if (
        (updatedData.textSize &&
          updatedData.textSize !== shapes[index].textSize) ||
        ...
        if (!updates.selectedShape) {
          ...
          updatedData.text = updates.selectedShape.text =
            updatedData.text || targetShape.text;
        }
      } else {
        const newData = { ...updatedData };

        if (
          typeof updatedData.x !== "undefined" &&
          typeof updatedData.y !== "undefined"
        ) {
      ...
      this.updateState(
        { shapes: latestShapes, ...updates, ...otherStatesToBeUpdated },
        () => {
  ...
  eraseAndRemoveShapeFromArray = (index, shapes) => {
    if (Object.keys(this.targettedShape(index)).length === 0) {
      return;
    }
  ...
  deleteShapeByKeyOrIndex = ({ index, key }, otherStates = {}) => {
    ...
    const shapeIndex =
      index || index === 0
        ? index
        : (this.getShapeByKeyOrIndex({ key }) || {}).index;
  ...
  deleteAllShapesOrByTypeOrIndicies = ({
    type,
    all = false,
    applied = false,
    secured = [],
    ...
    if (all) {
      const watermarkIndex = (
        this.getShapeByKeyOrIndex({ key: WATERMARK_UNIQUE_KEY }) || {}
      ).index;
      const securedIndicies = [...secured];
      if (
        (watermarkIndex || watermarkIndex === 0) &&
        !securedIndicies.includes(watermarkIndex)
      ) {
    ...
    if (shapesIndicies.length > 0) {
      this.deleteShapes(shapesIndicies);
    }
  };
    ...
    img.onload = () => {
      if (dataObject) {
        ...
        if (dataObject.position) {
          let x, y;
          [x, y, width, height] = getWatermarkSquaredPosition(
            dataObject.position,
            this._canvas,
            img.width,
            img.height
        ...
        const [originalWidth, originalHeight] = this.getSuitableImgDiemensions(
          img,
          dataObject.lockScaleToPercentage
        );
  ...
  fromLatestCanvasSizeValue = (number, property) => {
    if (this._canvas && this.state.latestCanvasSize) {
      ...
      return number.mapNumber(
        0,
        this.state.latestCanvasSize[property],
        0,
        this._canvas[property]
      );
    }
